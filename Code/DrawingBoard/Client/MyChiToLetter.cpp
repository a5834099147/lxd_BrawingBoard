...
string MyChiToLetter::GetLetter(const string& strText)
{
    ...
    for (int i=0; i<strText.length(); i++)
    {
        ...
        if (ucHigh < 0x80)
        {
            strLetters = strLetters + strText[i];
            if(m_LetterEnd) 
            {
        ...
        strLetters=strLetters+strRes;
        i++;
        if(m_LetterEnd) strLetters=strLetters + '\'';
    }
    return strLetters;
}
string MyChiToLetter::FindLetter( int nCode )
{
    string strValue;
    switch(nCode)
    {
